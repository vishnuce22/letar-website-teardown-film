...
//
// Phase A (0–35%):  panels loop at 0.9 / 1.0 / 1.1 playbackRate — independent.
// Phase B (35–70%): rates ease to 1.0 and loop phases lock (PLL-style gentle
//                   rate corrections — never currentTime seeks, no stutter).
// Phase C (70–100%): dividers slide away, panels crossfade to the merge clip.
//
...
if (section && active) {
  ...
  function apply() {
    ...
    const tb = ease(clamp((p - 0.35) / 0.35, 0, 1))
    ...
    const tc = ease(clamp((p - 0.7) / 0.3, 0, 1))
    ...
    if (finalCap) finalCap.classList.toggle('on', tc > 0.6)
  }
